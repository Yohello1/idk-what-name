...
SDL_Window *window;
// je ne comprends pas que est ce fait, mais c'est important
SDL_Event event;
enum pixel_state
{
    empty,
...
// ACTUALLY THIS IS ONLY NEEDED FOR WATER
struct cord_2d
{
    int x_pos;
    int y_pos;
...
int main()
{
    ...
    // This just throws points in the center as fire
    ...
    // wtf is this
    // Sets random points as non-flameable 
    for (int x_pos = 0; x_pos < LOGICAL_WINDOW_WIDTH; x_pos++)
    {
        for (int y_pos = 0; y_pos < LOGICAL_WINDOW_WIDTH; y_pos++)
        {
    ...
    // Bias, so we go like line left, and cw
    int bias = 90, power = 10;
    ...
    for (int i = 0; i < 2000; i++)
    {
        ...
        for (int x_pos = 0; x_pos < LOGICAL_WINDOW_WIDTH; x_pos++)
        {
	      ...
	      // increases temp
		    ...
		    // Gonna do some fancy gymnastics to get this to work
                    /*
                     . 3 .
                     2 X 4
                     . 1 .
                    */
                    if ( y_pos != 255
		      && pixels[x_pos][y_pos + 1].flameability == flameable
		      && pixels[x_pos][y_pos + 1].temperature != 510)
                    {
		      // Gonna actually yeet this, because flashpoints but I'll redo it later
                        pixels[x_pos][y_pos + 1].temperature += 1;
                        pixels[x_pos][y_pos].r = pixels[x_pos][y_pos].temperature /2 ;
                      
		    }
			if (x_pos != 1
		    &&  pixels[x_pos - 1][y_pos].flameability == flameable
		    &&  pixels[x_pos - 1][y_pos].temperature != 510)
                    {
...
// Ya know that one rock song I forgot the name of, it's like "forget" smth AHHHH
void cellular_automata(x_pos, y_pos){
}
